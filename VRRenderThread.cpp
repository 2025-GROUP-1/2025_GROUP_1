...
private:
    ...
    void executeAction(const VRMenu::Button& btn)
    {
        ...
        switch (btn.action) {
        case VRMenu::Clip: {
            bool newState = !part->m_clipEnabled;
            part->m_clipEnabled = newState;
            part->rebuildVRPipeline();
            if (vrThread)
                emit vrThread->partClipChanged(menu.targetPartID, newState);
            break;
        }
        case VRMenu::Shrink: {
            bool newState = !part->m_shrinkEnabled;
            part->m_shrinkEnabled = newState;
            part->rebuildVRPipeline();
            if (vrThread)
                emit vrThread->partShrinkChanged(menu.targetPartID, newState);
            break;
        }
        case VRMenu::ColRed:
    ...
    void applyColour(ModelPart* part, const QColor& c)
    {
        ...
        if (vrThread)
            emit vrThread->partColourChanged(menu.targetPartID, c);
    }
};
...
void VRRenderThread::run() {
    ...
    // must be after Initialize() so the OpenGL context exists for textures
    QString garageGlb = QCoreApplication::applicationDirPath() + "/garage__warehouse.glb";
    if (!QFileInfo::exists(garageGlb))
        garageGlb = QCoreApplication::applicationDirPath() + "/garage.glb";

    vtkNew<vtkGLTFImporter> envImporter;
    bool garageLoaded = false;

    if (QFileInfo::exists(garageGlb)) {
        envImporter->SetFileName(garageGlb.toStdString().c_str());
        envImporter->SetRenderWindow(m_renderWindow);
        envImporter->Update();
        garageLoaded = true;
    }
